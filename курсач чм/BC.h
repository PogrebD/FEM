...
public:
	...
	void primeniaemKraevble1(Grid _Grid, GlobalMatrix& _GlobalMatrix, int Nbc2, int Nbc3, int Nbc1, int Nnode, double w)
	{
		double h;
		// 2
		double theta;
		...
		for (int i = 0; i < Nbc2; i++)
		{
			...
			{
				h = _Grid.width;
				theta = Cfg::bc2(_Grid.Nodes[BC2vector[i].versh[0]].r, _Grid.Nodes[BC2vector[i].versh[0]].z, _Grid._Time.timeSloy[w]);
				if (BC2vector[i].versh[0] < Cfg::xElem)
					theta = -1 * Cfg::bc2(_Grid.Nodes[BC2vector[i].versh[0]].r, _Grid.Nodes[BC2vector[i].versh[0]].z, _Grid._Time.timeSloy[w]);
				
				_GlobalMatrix.Globalvector[BC2vector[i].versh[0]] += ((h * _Grid.Nodes[BC2vector[i].versh[0]].r * (theta * 2 + theta)) / 6) + ((h * h * (theta + theta)) / 12);
				_GlobalMatrix.Globalvector[BC2vector[i].versh[1]] += ((h * _Grid.Nodes[BC2vector[i].versh[0]].r * (theta * 2 + theta)) / 6) + ((h * h * (theta + theta * 3)) / 12);
			}
			else
			{
				h = _Grid.height;
		...
		for (int i = 0; i < Nbc1; i++)
		{
			for (int k = 0; k < 2; k++)
			{
				if (BC1vector[i].versh[k] != 0)
					...
					{
						_GlobalMatrix.Globalvector[_GlobalMatrix.jg[j]] -= Cfg::U(_Grid.Nodes[BC1vector[i].versh[k]].r, _Grid.Nodes[BC1vector[i].versh[k]].z, _Grid._Time.timeSloy[w]) * _GlobalMatrix.GlobalGMtriangle[j];
						_GlobalMatrix.GlobalGMtriangle[j] = 0;
					}
				_GlobalMatrix.Globalvector[BC1vector[i].versh[k]] = Cfg::U(_Grid.Nodes[BC1vector[i].versh[k]].r, _Grid.Nodes[BC1vector[i].versh[k]].z, _Grid._Time.timeSloy[w]);
				...
				for (int j = BC1vector[i].versh[k] + 1; j < Nnode; j++)
				{
					for (int h = _GlobalMatrix.ig[j - 1]; h < _GlobalMatrix.ig[j]; h++)
					{
						...
						{
							_GlobalMatrix.Globalvector[j] -= Cfg::U(_Grid.Nodes[BC1vector[i].versh[k]].r, _Grid.Nodes[BC1vector[i].versh[k]].z, _Grid._Time.timeSloy[w]) * _GlobalMatrix.GlobalGMtriangle[h];
	...
	void primeniaemKraevble2(Grid _Grid, GlobalMatrix& _GlobalMatrix, vector<double> GlobalGMtriangle, vector<double> GlobalGMdiag, int Nbc2, int Nbc3, int Nbc1, int Nnode, double w)
	{
		double h;
		// 2
		for (int i = 0; i < Nbc2; i++)
		...
		for (int i = 0; i < Nbc3; i++)
		{
			if (BC3vector[i].versh[1] - BC3vector[i].versh[0] == 1)
			{
				...
				{
					if (_GlobalMatrix.jg[j] == BC3vector[i].versh[0])
						GlobalGMtriangle[j] += ((h * _Grid.Nodes[BC3vector[i].versh[0]].r * BC3vector[i].Beta) / 6) + ((h * h * BC3vector[i].Beta) / 12);
				}

				GlobalGMdiag[BC3vector[i].versh[0]] += ((h * _Grid.Nodes[BC3vector[i].versh[0]].r * BC3vector[i].Beta) / 3) + ((h * h * BC3vector[i].Beta) / 12);
				GlobalGMdiag[BC3vector[i].versh[1]] += ((h * _Grid.Nodes[BC3vector[i].versh[0]].r * BC3vector[i].Beta) / 3) + ((h * h * BC3vector[i].Beta) / 4);
			...
			else
			{
				...
				{
					if (_GlobalMatrix.jg[j] == BC3vector[i].versh[0])
						GlobalGMtriangle[j] += (h * _Grid.Nodes[BC3vector[i].versh[0]].r * BC3vector[i].Beta) / 6;
				}

				for (int j = 0; j < 2; j++)
				{
					GlobalGMdiag[BC3vector[i].versh[j]] += (h * _Grid.Nodes[BC3vector[i].versh[0]].r * BC3vector[i].Beta) / 3;
				}
			}
		...
		for (int i = 0; i < Nbc1; i++)
		{
			for (int k = 0; k < 2; k++)
			{
				if (BC1vector[i].versh[k] != 0)
					for (int j = _GlobalMatrix.ig[BC1vector[i].versh[k] - 1]; j < _GlobalMatrix.ig[BC1vector[i].versh[k]]; j++)
					{
						_GlobalMatrix.Globalvector[_GlobalMatrix.jg[j]] -= Cfg::U(_Grid.Nodes[BC1vector[i].versh[k]].r, _Grid.Nodes[BC1vector[i].versh[k]].z, _Grid._Time.timeSloy[w]) * GlobalGMtriangle[j];
						GlobalGMtriangle[j] = 0;
					}
				_GlobalMatrix.Globalvector[BC1vector[i].versh[k]] = Cfg::U(_Grid.Nodes[BC1vector[i].versh[k]].r, _Grid.Nodes[BC1vector[i].versh[k]].z, _Grid._Time.timeSloy[w]);
				GlobalGMdiag[BC1vector[i].versh[k]] = 1;
				vector<double> ystal;
				for (int j = BC1vector[i].versh[k] + 1; j < Nnode; j++)
				{
					for (int h = _GlobalMatrix.ig[j - 1]; h < _GlobalMatrix.ig[j]; h++)
					{
						if (_GlobalMatrix.jg[h] == BC1vector[i].versh[k])
						{
							_GlobalMatrix.Globalvector[j] -= Cfg::U(_Grid.Nodes[BC1vector[i].versh[k]].r, _Grid.Nodes[BC1vector[i].versh[k]].z, _Grid._Time.timeSloy[w]) * GlobalGMtriangle[h];
							GlobalGMtriangle[h] = 0;
						}
